...
def programstart():
    ...
    for row in olvasas:
        ...
        latinkicsi = (translit(forditas.decode(encoding='utf-8', errors='ignore'), language, reversed=True)).lower()
        latinnagy = (translit(forditas.decode(encoding='utf-8', errors='ignore'), language, reversed=True)).upper()
        latincapital = (translit(forditas.decode(encoding='utf-8', errors='ignore'), language, reversed=True)).title()
        cirillnagy = (translit(forditas.decode(encoding='utf-8', errors='ignore'), language, reversed=False)).upper()
        cirillkicsi = (translit(forditas.decode(encoding='utf-8', errors='ignore'), language, reversed=False)).lower()
        cirillcapital = (translit(forditas.decode(encoding='utf-8', errors='ignore'), language, reversed=False)).title()
